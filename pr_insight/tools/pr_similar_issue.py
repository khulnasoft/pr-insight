...
class PRSimilarIssue:
    def __init__(self, issue_url: str, ai_handler, args: list = None):
        ...
        index_name = self.index_name = "pr-insight-insight-issues"
        ...
        for issue in issues_list:
            ...
            if len(issue_str) < 8000 or self.token_handler.count_tokens(issue_str) < get_max_tokens(MODEL):  # fast reject first
                issue_record = Record(
                    id=issue_key + "." + "issue",
                    text=issue_str,
                    metadata=Metadata(repo=repo_name_for_index, username=username, created_at=created_at, level=IssueLevel.ISSUE),
                )
