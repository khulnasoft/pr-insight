...
class PRReviewer:
    ...
    def __init__(self, pr_url: str, is_answer: bool = False, is_auto: bool = False, args: list = None, ai_handler: partial[BaseAiHandler,] = LiteLLMAIHandler):
        ...
        self.token_handler = TokenHandler(self.git_provider.pr, self.vars, get_settings().pr_review_prompt.system, get_settings().pr_review_prompt.user)
    ...
    def _prepare_pr_review(self) -> str:
        ...
        data = load_yaml(
            ...
            keys_fix_yaml=["ticket_compliance_check", "estimated_effort_to_review_[1-5]:", "security_concerns:", "key_issues_to_review:", "relevant_file:", "relevant_line:", "suggestion:"],
            first_key=first_key,
            last_key=last_key,
        )
        ...
        markdown_text = convert_to_markdown_v2(data, self.git_provider.is_supported("gfm_markdown"), incremental_review_markdown_text, git_provider=self.git_provider, files=self.git_provider.get_diff_files())
    ...
    def _can_run_incremental_review(self) -> bool:
        ...
        recent_commits_threshold = datetime.datetime.now() - datetime.timedelta(minutes=get_settings().pr_reviewer.minimal_minutes_for_incremental_review)
        last_seen_commit_date = self.incremental.last_seen_commit.commit.author.date if self.incremental.last_seen_commit else None
        all_commits_too_recent = last_seen_commit_date > recent_commits_threshold if self.incremental.last_seen_commit else False
    ...
    def set_review_labels(self, data):
        ...
        if get_settings().pr_reviewer.enable_review_labels_security or get_settings().pr_reviewer.enable_review_labels_effort:
            try:
                review_labels = []
                ...
                if current_labels:
                    current_labels_filtered = [label for label in current_labels if not label.lower().startswith("review effort [1-5]:") and not label.lower().startswith("possible security concern")]
                else:
    ...
    def auto_approve_logic(self):
        ...
        if get_settings().pr_reviewer.enable_auto_approval:
                ...
                for label in current_labels:
                    if label.lower().startswith("review effort [1-5]:"):
                        ...
                        if effort > maximal_review_effort:
                            get_logger().info(f"Auto-approve error: PR review effort ({effort}) is higher than the maximal review effort ({maximal_review_effort}) allowed")
                            self.git_provider.publish_comment(f"Auto-approve error: PR review effort ({effort}) is higher than the maximal review effort ({maximal_review_effort}) allowed")
                            return
            ...
            self.git_provider.publish_comment(
                "Auto-approval option for PR-Insight is disabled. You can enable it via a [configuration file](https://github.com/khulnasoft/pr-insight/blob/main/docs/REVIEW.md#auto-approval-1)"
            )
