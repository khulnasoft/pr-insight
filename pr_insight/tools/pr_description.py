...
from pr_insight.algo.pr_processing import (OUTPUT_BUFFER_TOKENS_HARD_THRESHOLD,
                                           get_pr_diff,
                                           get_pr_diff_multiple_patchs,
                                           retry_with_fallback_models)
from pr_insight.algo.token_handler import TokenHandler
from pr_insight.algo.utils import (ModelType, PRDescriptionHeader, clip_tokens,
                                   get_max_tokens, get_user_labels, load_yaml,
                                   set_custom_labels,
                                   show_relevant_configurations)
from pr_insight.config_loader import get_settings
from pr_insight.git_providers import (GithubProvider, get_git_provider,
...
class PRDescription:
    ...
    async def run(self):
        try:
            ...
            relevant_configs = {"pr_description": dict(get_settings().pr_description), "config": dict(get_settings().config)}
            ...
            if get_settings().pr_description.use_description_markers:
                ...
                if not self.git_provider.is_supported("publish_file_comments") or not get_settings().pr_description.inline_file_summary:
                    pr_body += "\n\n" + changes_walkthrough
            ...
            get_logger().error(f"Error generating PR description {self.pr_id}: {e}", artifact={"traceback": traceback.format_exc()})

        return ""

    async def _prepare_prediction(self, model: str) -> None:
        if get_settings().pr_description.use_description_markers and "pr_insight:" not in self.user_description:
        ...
        if not large_pr_handling or patches_diff:
            ...
            if patches_diff:
                ...
                get_logger().error(f"Error getting PR diff {self.pr_id}", artifact={"traceback": traceback.format_exc()})
                self.prediction = None
        else:
            ...
            (patches_compressed_list, total_tokens_list, deleted_files_list, remaining_files_list, file_dict, files_in_patches_list) = get_pr_diff_multiple_patchs(
            ...
            if not get_settings().pr_description.async_ai_calls:
                ...
                for i, patches in enumerate(patches_compressed_list):  # sync calls
                    ...
                    prediction_files = await self._get_prediction(model, patches_diff, prompt="pr_description_only_files_prompts")
                ...
                for i, patches in enumerate(patches_compressed_list):
                    if patches:
                        patches_diff = "\n".join(patches)
                        get_logger().debug(f"PR diff number {i + 1} for describe files")
                        task = asyncio.create_task(self._get_prediction(model, patches_diff, prompt="pr_description_only_files_prompts"))
                        tasks.append(task)
            ...
            token_handler_only_description_prompt = TokenHandler(
                self.git_provider.pr, self.vars, get_settings().pr_description_only_description_prompts.system, get_settings().pr_description_only_description_prompts.user
            )
            ...
            if deleted_files_list:
                ...
                for i, file in enumerate(deleted_files_list):
                    ...
                    if i >= MAX_EXTRA_FILES_TO_PROMPT:
                        ...
                        break
            tokens_files_walkthrough = len(token_handler_only_description_prompt.encoder.encode(files_walkthrough_prompt))
            ...
            if total_tokens > max_tokens_model - OUTPUT_BUFFER_TOKENS_HARD_THRESHOLD:
                ...
                files_walkthrough_prompt = clip_tokens(
                    files_walkthrough_prompt, max_tokens_model - OUTPUT_BUFFER_TOKENS_HARD_THRESHOLD - token_handler_only_description_prompt.prompt_tokens, num_input_tokens=tokens_files_walkthrough
                )

            # PR header inference
            get_logger().debug(f"PR diff only description", artifact=files_walkthrough_prompt)
            prediction_headers = await self._get_prediction(model, patches_diff=files_walkthrough_prompt, prompt="pr_description_only_description_prompts")
...
"""
            ...
            if counter_extra_files > 0:
                ...
                if original_prediction_dict and isinstance(original_prediction_dict, dict) and isinstance(prediction_extra_dict, dict) and "pr_files" in prediction_extra_dict:
...
"""
    ...
    def _prepare_pr_answer_with_markers(self) -> Tuple[str, str, str, List[dict]]:
        ...
        if ai_walkthrough and not re.search(r"<!--\s*pr_insight:walkthrough\s*-->", body):
            try:
                walkthrough_gfm, pr_file_changes = self.process_pr_files_prediction(walkthrough_gfm, self.file_label_dict)
        ...
        for file in self.data["pr_files"]:
            try:
                ...
                if not all(field in file for field in required_fields):
                    ...
                    get_logger().warning(f"Missing required fields in file label dict {self.pr_id}, skipping file", artifact={"file": file})
                    continue
                if not file.get("changes_title"):
                    get_logger().warning(f"Empty changes title or summary in file label dict {self.pr_id}, skipping file", artifact={"file": file})
                    continue
        ...
        try:
            ...
            for semantic_label in value.keys():
                ...
                for filename, file_changes_title, file_change_description in list_tuples:
                    ...
                    pr_body = self.add_file_data(delta_nbsp, diff_plus_minus, file_change_description_br, filename, filename_publish, link, pr_body)
    ...
    def add_file_data(self, delta_nbsp, diff_plus_minus, file_change_description_br, filename, filename_publish, link, pr_body) -> str:
