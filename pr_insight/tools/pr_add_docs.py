...
class PRAddDocs:
    def __init__(self, pr_url: str, cli_mode=False, args: list = None, ai_handler: partial[BaseAiHandler,] = LiteLLMAIHandler):
        ...
        self.token_handler = TokenHandler(self.git_provider.pr, self.vars, get_settings().pr_add_docs_prompt.system, get_settings().pr_add_docs_prompt.user)
    ...
    def push_inline_docs(self, data):
        ...
        for d in data["Code Documentation"]:
            try:
                ...
                if documentation:
                    new_code_snippet = self.dedent_code(relevant_file, relevant_line, documentation, doc_placement, add_original_line=True)

                    body = f"**Suggestion:** Proposed documentation\n```suggestion\n" + new_code_snippet + "\n```"
                    docs.append({"body": body, "relevant_file": relevant_file, "relevant_lines_start": relevant_line, "relevant_lines_end": relevant_line})
            except Exception:
                if get_settings().config.verbosity_level >= 2:
