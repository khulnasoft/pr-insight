...
def extend_patch(original_file_str, patch_str, patch_extra_lines_before=0, patch_extra_lines_after=0, filename: str = "", new_file_str="") -> str:
    ...
    try:
        extended_patch_str = process_patch_lines(patch_str, original_file_str, patch_extra_lines_before, patch_extra_lines_after, new_file_str)
        ...
        for i, line in enumerate(patch_lines):
            if line.startswith("@@"):
                ...
                if match:
                    ...
                    if is_valid_hunk and (patch_extra_lines_before > 0 or patch_extra_lines_after > 0):
                        ...
                        if allow_dynamic_context and file_new_lines:
                            ...
                            if lines_before_original == lines_before_new:  # Making sure no changes from a previous hunk
                                for (
                                    i,
                                    line,
                                ) in enumerate(lines_before_original):
                                ...
                                get_logger().debug(
                                    f"Extra lines before hunk are different in original and new file - dynamic context",
                                    artifact={"lines_before_original": lines_before_original, "lines_before_new": lines_before_new},
                                )

                            if not found_header:
                                # get_logger().debug(f"Section header not found in the extra lines before the hunk")
                                extended_start1, extended_size1, extended_start2, extended_size2 = _calc_context_limits(patch_extra_lines_before)
                        else:
                            extended_start1, extended_size1, extended_start2, extended_size2 = _calc_context_limits(patch_extra_lines_before)
                        ...
                        if file_new_lines:
                            ...
                            if delta_lines_original != delta_lines_new:
                                get_logger().debug(
                                    f"Extra lines before hunk are different in original and new file", artifact={"delta_lines_original": delta_lines_original, "delta_lines_new": delta_lines_new}
                                )
                    ...
                    extended_patch_lines.append(f"@@ -{extended_start1},{extended_size1} +{extended_start2},{extended_size2} @@ {section_header}")
    ...
    try:
        if i + 1 < len(patch_lines) and patch_lines[i + 1][0] == " ":  # an existing line in the file
            if patch_lines[i + 1].strip() != original_lines[start1 - 1].strip():
                ...
                is_valid_hunk = False
                get_logger().info(f"Invalid hunk in PR, line {start1} in hunk header doesn't match the original file content")
    except:
        pass
...
def handle_patch_deletions(patch: str, original_file_content_str: str, new_file_content_str: str, file_name: str, edit_type: EDIT_TYPE = EDIT_TYPE.UNKNOWN) -> str:
    """
