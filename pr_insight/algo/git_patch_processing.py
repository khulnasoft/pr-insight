...
def extend_patch(original_file_str, patch_str, patch_extra_lines_before=0, patch_extra_lines_after=0, filename: str = "") -> str:
    ...
    try:
        extended_patch_str = process_patch_lines(patch_str, original_file_str, patch_extra_lines_before, patch_extra_lines_after)
        ...
        for i, line in enumerate(patch_lines):
            if line.startswith("@@"):
                ...
                if match:
                    ...
                    extended_patch_lines.append(f"@@ -{extended_start1},{extended_size1} " f"+{extended_start2},{extended_size2} @@ {section_header}")
    ...
    try:
        if i + 1 < len(patch_lines) and patch_lines[i + 1][0] == " ":  # an existing line in the file
            if patch_lines[i + 1].strip() != original_lines[start1 - 1].strip():
                is_valid_hunk = False
                get_logger().error(f"Invalid hunk in PR, line {start1} in hunk header doesn't match the original file content")
    except:
        pass
