...
class LocalGitProvider(GitProvider):
    ...
    def __init__(self, target_branch_name, incremental=False):
        ...
        self.description_path = get_settings().get("local.description_path") if get_settings().get("local.description_path") is not None else self.repo_path / "description.md"
        self.review_path = get_settings().get("local.review_path") if get_settings().get("local.review_path") is not None else self.repo_path / "review.md"
    ...
    def get_diff_files(self) -> list[FilePatchInfo]:
        diffs = self.repo.head.commit.diff(self.repo.merge_base(self.repo.head, self.repo.branches[self.target_branch_name]), create_patch=True, R=True)
        diff_files = []
    ...
    def get_files(self) -> List[str]:
        ...
        """
        diff_index = self.repo.head.commit.diff(self.repo.merge_base(self.repo.head, self.repo.branches[self.target_branch_name]), R=True)
