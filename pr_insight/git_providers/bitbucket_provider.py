...
class BitbucketProvider(GitProvider):
    ...
    def get_repo_settings(self):
        try:
            url = f"https://api.bitbucket.org/2.0/repositories/{self.workspace_slug}/{self.repo_slug}/src/{self.pr.destination_branch}/.pr_insight.toml"
    ...
    def is_supported(self, capability: str) -> bool:
        if capability in ["get_issue_comments", "publish_inline_comments", "get_labels", "gfm_markdown", "publish_file_comments"]:
            return False
        return True
    ...
    def get_diff_files(self) -> list[FilePatchInfo]:
        ...
        if diffs != diffs_original:
            try:
                ...
                get_logger().info(f"Filtered out [ignore] files for PR", extra={"original_files": names_original, "names_kept": names_kept, "names_filtered": names_filtered})
            except Exception as e:
                pass
        ...
        #   --- a/pr_insight/cli_pip.py
        #  +++ b/pr_insight/cli_pip.py
        #   @@ -... @@"
        for i, _ in enumerate(diff_split):
            diff_split_lines = diff_split[i].splitlines()
            if (len(diff_split_lines) >= 6) and (
                (diff_split_lines[2].startswith("---") and diff_split_lines[3].startswith("+++") and diff_split_lines[4].startswith("@@"))
                or (
            ...
            try:
                ...
                if get_settings().get("bitbucket_app.avoid_full_files", False):
                    ...
                    if counter_valid == MAX_FILES_ALLOWED_FULL // 2:
                        get_logger().info(f"Bitbucket too many files in PR, will avoid loading full content for rest of files")
                    original_file_content_str = ""
                    new_file_content_str = ""
            except Exception as e:
            ...
            for comment in self.pr.comments():
                ...
                if initial_header in body:
                    ...
                    if final_update_message:
                        self.publish_comment(f"**[Persistent {name}]({comment_url})** updated to latest commit {latest_commit_url}")
                    return
        except Exception as e:
    ...
    def get_pr_file_content(self, file_path: str, branch: str) -> str:
        try:
            ...
            url = f"https://api.bitbucket.org/2.0/repositories/{self.workspace_slug}/{self.repo_slug}/src/{branch}/{file_path}"
            response = requests.request("GET", url, headers=self.headers)
            if response.status_code == 404:  # not found
                return ""
