...
from pr_insight.git_providers.local_git_provider import PullRequestMimic
...
def adopt_to_gerrit_message(message):
    ...
    for line in lines:
        ...
        line = line.replace("*", "").replace("``", "`").replace("<details>", "").replace("</details>", "").replace("<summary>", "").replace("</summary>", "")
...
class GerritProvider(GitProvider):
    def __init__(self, key: str, incremental=False):
        ...
        self.parsed_url = urllib3.util.parse_url(f"{parsed.scheme}://{user}@{parsed.host}:{parsed.port}")
    ...
    def get_pr_labels(self, update=False):
        raise NotImplementedError("Getting labels is not implemented for the gerrit provider")

    def add_eyes_reaction(self, issue_comment_id: int, disable_eyes: bool = False):
        raise NotImplementedError("Adding reactions is not implemented for the gerrit provider")

    def remove_reaction(self, issue_comment_id: int, reaction_id: int):
        raise NotImplementedError("Removing reactions is not implemented for the gerrit provider")
    ...
    def get_repo_settings(self):
        try:
            with open(self.repo_path / ".pr_insight.toml", "rb") as f:
                contents = f.read()
            return contents
        except OSError:
            return b""
    ...
    def publish_inline_comments(self, comments: list[dict]):
        raise NotImplementedError("Publishing inline comments is not implemented for the gerrit " "provider")
