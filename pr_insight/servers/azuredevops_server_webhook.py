...
# The server listens for incoming webhooks from Azure DevOps Server and forwards them to the PR Insight.
...
async def handle_webhook(background_tasks: BackgroundTasks, request: Request):
    ...
    if data["eventType"] == "git.pullrequest.created":
        ...
        await _perform_commands_azure("pr_commands", PRInsight(), pr_url, log_context)
        return
        ...
        if available_commands_rgx.match(data["resource"]["comment"]["content"]):
            if data["resourceVersion"] == "2.0":
                ...
                return (
                    JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content=json.dumps({"message": "version 1.0 webhook for Azure Devops PR comment is not supported. please upgrade to version 2.0"})),
                )
        else:
            return JSONResponse(
    ...
    return JSONResponse(status_code=status.HTTP_202_ACCEPTED, content=jsonable_encoder({"message": "webhook triggered successfully"}))
