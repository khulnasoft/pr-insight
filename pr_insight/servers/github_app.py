...
async def handle_comments_on_pr(body: Dict[str, Any], event: str, sender: str, sender_id: str, action: str, log_context: Dict[str, Any], insight: PRInsight):
    ...
    if "issue" in body and "pull_request" in body["issue"] and "url" in body["issue"]["pull_request"]:
        ...
        try:
            if "/ask" in comment_body and "subject_type" in body["comment"] and body["comment"]["subject_type"] == "line":
    ...
    with get_logger().contextualize(**log_context):
        if get_identity_provider().verify_eligibility("github", sender_id, api_url) is not Eligibility.NOT_ELIGIBLE:
            ...
            await insight.handle_request(api_url, comment_body, notify=lambda: provider.add_eyes_reaction(comment_id, disable_eyes=disable_eyes))
        else:
            get_logger().info(f"User {sender=} is not eligible to process comment on PR {api_url=}")


async def handle_new_pr_opened(body: Dict[str, Any], event: str, sender: str, sender_id: str, action: str, log_context: Dict[str, Any], insight: PRInsight):
    ...
    if action in get_settings().github_app.handle_pr_actions:  # ['opened', 'reopened', 'ready_for_review']
        ...
        if get_identity_provider().verify_eligibility("github", sender_id, api_url) is not Eligibility.NOT_ELIGIBLE:
            await _perform_auto_commands_github("pr_commands", insight, body, api_url, log_context)
        else:
            get_logger().info(f"User {sender=} is not eligible to process PR {api_url=}")


async def handle_push_trigger_for_new_commits(body: Dict[str, Any], event: str, sender: str, sender_id: str, action: str, log_context: Dict[str, Any], insight: PRInsight):
    pull_request, api_url = _check_pull_request_event(action, body, log_context)
    if not (pull_request and api_url):
        return {}
    ...
    try:
        if get_identity_provider().verify_eligibility("github", sender_id, api_url) is not Eligibility.NOT_ELIGIBLE:
            ...
            await _perform_auto_commands_github("push_commands", insight, body, api_url, log_context)
...
def handle_closed_pr(body, event, action, log_context):
    ...
    log_context["api_url"] = api_url
    get_logger().info("PR-Insight statistics for closed PR", analytics=True, pr_statistics=pr_statistics, **log_context)
...
async def handle_request(body: Dict[str, Any], event: str):
    ...
    # handle comments on PRs
        ...
        await handle_comments_on_pr(body, event, sender, sender_id, action, log_context, insight)
    # handle new PRs
    elif event == "pull_request" and action != "synchronize" and action != "closed":
        get_logger().debug(f"Request body", artifact=body, event=event)
        await handle_new_pr_opened(body, event, sender, sender_id, action, log_context, insight)
    elif event == "issue_comment" and "edited" in action:
        pass  # handle_checkbox_clicked
    # handle pull_request event with synchronize action - "push trigger" for new commits
    elif event == "pull_request" and action == "synchronize":
        await handle_push_trigger_for_new_commits(body, event, sender, sender_id, action, log_context, insight)
...
async def _perform_auto_commands_github(commands_conf: str, insight: PRInsight, body: dict, api_url: str, log_context: dict):
    apply_repo_settings(api_url)
