...
def test_e2e_run_github_app():
    """
    What we want to do:
    (1) open a PR in a repo 'https://github.com/khulnasoft/pr-insight-tests'
...
def test_e2e_run_nextjs_vercel():
    """
    What we want to do:
    (1) open a PR in a repo 'https://github.com/khulnasoft/pr-insight-tests'
    (2) wait for 5 minutes until the PR is processed by the GitHub app
    ...
    """
    base_branch = "main"  # or any base branch you want
    new_branch = f"nextjs_vercel_e2e_test-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}"
    repo_url = 'khulnasoft/pr-insight-tests'
    get_settings().config.git_provider = "github"
    git_provider = get_git_provider()()
    github_client = git_provider.github_client
    ...
    try:
        ...
        # If we reach here, the test is successful
        logger.info(f"Succeeded in running e2e test for Next.js and Vercel on the PR {pr.html_url}")
    except Exception as e:
        logger.error(f"Failed to run e2e test for Next.js and Vercel: {e}")
        # delete the branch
        logger.info(f"Deleting the branch {new_branch}")
        repo.get_git_ref(f"heads/{new_branch}").delete()
        assert False
